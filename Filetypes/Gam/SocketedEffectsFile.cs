...
{
    public static class SocketedEffectsIO
    {
        public static SocketedEffectsJsonFile ReadGamFile(string filePath)
        {
            ...
            if (blockOff == 0 || blockLen <= 0)
            {
                ...
                if (blockLen <= 0) throw new InvalidDataException("SocketedEffects block pointer invalid.");
            }
            ...
            if (A == 0) A = 24; // common value

            var recs = new List<SocketedEffectRecord>();
            int stride = 256 + 16 + 5 * A + 1024; // Name[256] + 4 ints + 5*A + S0[1024]
            ...
            return new SocketedEffectsJsonFile { Header = outHeader, AttrSize = A, Records = recs };
        }

        public static void WriteGamFile(string filePath, SocketedEffectsJsonFile data)
        {
            ...
            int A = data.AttrSize > 0 ? data.AttrSize :
                    data.Records?.FirstOrDefault()?.Attribute?.FirstOrDefault()?.Length ??
                     data.Records?.FirstOrDefault()?.ReqAttribute?.FirstOrDefault()?.Length ?? 24;

            foreach (var r in data.Records ?? Enumerable.Empty<SocketedEffectRecord>())
                WriteOne(bw, r, A);
        ...
        private static SocketedEffectRecord ReadOne(BinaryReader s, int A)
        {
            var r = new SocketedEffectRecord();
            r.Name = ReadFixedCString(s, 256);
            r.I0 = s.ReadInt32();
            r.I1 = s.ReadInt32();
            r.Item = s.ReadInt32();
            r.ItemType = s.ReadInt32();

            r.Attribute = new byte[3][];
            for (int i = 0; i < 3; i++) r.Attribute[i] = s.ReadBytes(A);

            r.ReqAttribute = new byte[2][];
            for (int i = 0; i < 2; i++) r.ReqAttribute[i] = s.ReadBytes(A);

            r.S0 = ReadFixedCString(s, 1024);
            return r;
        }

        private static void WriteOne(BinaryWriter w, SocketedEffectRecord r, int A)
        {
            WriteFixedCString(w, r?.Name ?? string.Empty, 256);
            w.Write(r?.I0 ?? 0);
            w.Write(r?.I1 ?? 0);
            w.Write(r?.Item ?? 0);
            w.Write(r?.ItemType ?? 0);

            var attr = r?.Attribute ?? Array.Empty<byte[]>();
            for (int i = 0; i < 3; i++)
            {
                var blob = i < attr.Length && attr[i] != null ? attr[i] : Array.Empty<byte>();
            ...
            var req = r?.ReqAttribute ?? Array.Empty<byte[]>();
            for (int i = 0; i < 2; i++)
            {
                var blob = i < req.Length && req[i] != null ? req[i] : Array.Empty<byte>();
                if (blob.Length == A) w.Write(blob);
                else if (blob.Length > A) w.Write(blob, 0, A);
                else { w.Write(blob); Bin.WriteZeros(w.BaseStream, A - blob.Length); }
            }
    ...
    public class SocketedEffectsJsonFile
    {
        public Header Header { get; set; } = Header.Default();
        public int AttrSize { get; set; } = 24; // byte length of one AttributeSpecifier blob
        public List<SocketedEffectRecord> Records { get; set; } = new List<SocketedEffectRecord>();
    }

    public class SocketedEffectRecord
    {
        ...
        public byte[][] Attribute { get; set; } = Array.Empty<byte[]>(); // 3 entries of A bytes
        public byte[][] ReqAttribute { get; set; } = Array.Empty<byte[]>(); // 2 entries of A bytes
        public string S0 { get; set; } = string.Empty; // 1024 bytes, null-terminated
    }
}
